...
void imprimir_alumno(const struct directorio *temp) {
    printf("Matricula: %d\n", temp->matricula);
    printf("Nombre: %s\n", temp->name);
...
void validar_numero(const char *mensaje, int *destino, int longitud_minima) {
    ...
    do {
        valido = 1;
        printf("%s", mensaje);
        scanf("%19s", buffer);  // Leer como cadena
        limpiar_buffer();
...
void busqueda_datos(const char *campo_buscado) {
    ...
    while (fread(&temp, sizeof(struct directorio), 1, archivo)) {
        ...
        if (strcmp(campo_buscado, "Nombre") == 0 && strstr(temp.name, busqueda)) coincidencia = 1;
        else if (strcmp(campo_buscado, "Apellido Paterno") == 0 && strstr(temp.apellido, busqueda)) coincidencia = 1;
        else if (strcmp(campo_buscado, "Apellido Materno") == 0 && strstr(temp.apellidoM, busqueda)) coincidencia = 1;
        else if (strcmp(campo_buscado, "Telefono") == 0) {
            char telefono_str[11];
            snprintf(telefono_str, sizeof(telefono_str), "%d", temp.tel); // convierte a cadena
            if (strstr(telefono_str, busqueda)) {
                coincidencia = 1;
            }
        }
...
void editar_datos() {
    ...
    while (fread(&temp, sizeof(struct directorio), 1, archivo)){
        if (temp.matricula == matricula) {
            ...
            fwrite(&temp, sizeof(struct directorio), 1, archivo);
            encontrado = 1;
...
void eliminar_datos() {
    ...
    // Intento de eliminación y renombrado
    if (remove("contactos.dat") == 0) {
        if (rename("temp.dat", "contactos.dat") != 0) {
            printf("\nError al renombrar. Recuperando datos...\n");
            // Recuperación de emergencia en caso de falla de eliminación.
            temp_archivo = fopen("temp.dat", "rb");
            archivo = fopen("contactos.dat", "wb");
            if (temp_archivo && archivo) {
                while (fread(&temp, sizeof(struct directorio), 1, temp_archivo) == 1) {
                    fwrite(&temp, sizeof(struct directorio), 1, archivo);
                }
            }
            if (temp_archivo) fclose(temp_archivo);
            if (archivo) fclose(archivo);
            remove("temp.dat");
        }
    } else {
        printf("\nNo se pudo eliminar el archivo original. Usando metodo alternativo...\n");
        // Método alternativo: copia directa
        archivo = fopen("contactos.dat", "wb");
        temp_archivo = fopen("temp.dat", "rb");
        
        if (archivo && temp_archivo) {
            while (fread(&temp, sizeof(struct directorio), 1, temp_archivo) == 1) {
                fwrite(&temp, sizeof(struct directorio), 1, archivo);
            }
            fclose(archivo);
            fclose(temp_archivo);
            remove("temp.dat");
            printf("Operacion completada usando copia directa\n");
        } else {
